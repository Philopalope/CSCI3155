object Lab1 extends jsy.util.JsyApplication {
  ...
  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double ={
    ...
    if(abs(x0*x0-c)>=epsilon) sqrtErr(c,sqrtStep(c,x0),epsilon)
    else x0
  }
  ...
  def repOk(t: SearchTree): Boolean = {
    def check(t: SearchTree, min: Int, max: Int): Boolean = t match {
      case Empty => true
      case Node(l, d, r) => if(d>=max||d<min) false 
      else {
  ...
  def delete(t: SearchTree, n: Int): SearchTree = {
    t match{
      case Node(l,d,r) if(n<d) => l match{
        case Empty => t
        case _=>new Node(delete(l,n),d,r)
      }
      case Node(l,d,r) if(n>d) => r match{
        case Empty => t
        case _=>new Node(l,d,delete(r,n))
      }
      case Node(l,n,r) =>r match{
        case Empty => l 
        case _ => {
