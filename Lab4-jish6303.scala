object Lab4 extends jsy.util.JsyApplication {
  ...
  def typeInfer(env: Map[String,Typ], e: Expr): Typ = {
    ...
    e match {
      ...
      case Function(p, params, tann, e1) => {
        ...
        tann match {
          case None => {
            ...
            val tO = TFunction(params, t)
            tO
          }
          case Some(tret) =>  {
            val t = typeInfer(env2, e1)
            val tO = TFunction(params, t)
            if (tO != TFunction(params, tret)) err(t, e1) else TFunction(params, tret)
          }
        }
      }
      ...
      case Obj(fields) =>TObj(fields.map { case (k, v) => (k, typ(v)) })
      case GetField(e1, f) =>{
        val e = typ(e1)
        e match {
  ...
  def substitute(e: Expr, v: Expr, x: String): Expr = {
    ...
    e match {
      ...
      case f1 @ Function(p, params, tann, e1) => {
        if (params.exists((t1: (String, Typ)) => t1._1 == x) || Some(x) == p) { f1 }
  ...
  def step(e: Expr): Expr = {
    ...
    e match {
      ...
      /*** Fill-in more cases here. ***/
      case Call(Function(name, params, t, e1), args) if args.forall(isValue) => {
        val x = name match {
          case Some(y) => substitute(e1, Function(name, params, t, e1), y)
          case None => e1
        }
        params.zip(args).foldLeft(x){
          (e2, t2) => substitute(e2, t2._2, t2._1._1)
        }
      }
      case Call(Function(name, params, t, e1), args) => {
        Call(Function(name, params, t, e1), 
          mapFirst((arg: Expr) => {
            if(isValue(arg))
              None
            else
